...
use crate::dsm_key_config::{ClientConnectionInfo, DsmFsOps, DsmInterface, EncryptedPassphrase};
use crate::utils::{run_subprocess, run_subprocess_with_output_setup, CommandOutputConfig};
...
/// the device
async fn get_key_file(conn_info: Option<ClientConnectionInfo<'_>>, device_path: &str) -> Result<bool, String> {
    ...
    match is_luks_device(device_path).await {
        ...
        Err(_) => {
            ...
            luks_format_device(key_path, device_path, DETACHED_HEADER_PATH).await?;
...
pub async fn mount_read_write_file_system(conn_info: Option<ClientConnectionInfo<'_>>, device_path: &str, mount_path: &str) -> Result<(), String> {
    ...
    let create_ext4 = get_key_file(conn_info, device_path).await?;
    ...
    run_mount(&[&dm_crypt_mapped_device, mount_path]).await?;
