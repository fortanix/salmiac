...
const CERT_RENEWAL_BEFORE_EXPIRY: Duration =
    Duration::from_secs(10 * 60 * 60 * 24 /* 10 days */);
...
pub(crate) async fn run(
    ...
    let cert_settings = setup_result
        .enclave_manifest
        .user_config
        .certificate_config
        .clone();
    ...
    let enclave_exit_code = with_background_tasks!(background_tasks, {
        ...
        let encrypted_fs = setup_file_system(
            parent_guard.deref_mut(),
            FileSystemSetupApiImpl {},
            fs_setup_config,
        )
        .await?;
...
async fn signal_user_program_exit_status(
    ...
    let exit_status = exit_status.map_err(|err| EnclaveErrorCode::enclave_failure(err, is_debug));
    match parent
...
pub(crate) async fn setup_enclave_certifications<
    Socket: AsyncWrite + AsyncRead + Unpin + Send,
    Api: CSRApi,
>(
    vsock: &mut Socket,
...
>(
    ...
    // the two should be combined into one list of names - First name is the Subject and
    // the complete list are included in the SAN Extension.
...
"
    ...
    fn check_enclave_env_vars() {
        ...
        let test_cases = vec![
            TestCase {
                name: "runtime overrides manifest env",
        ...
        for testc in test_cases {
            let transform_to_strings = |env_vars: Vec<(&str, &str)>| {
                env_vars
                    .into_iter()
...
fn get_fs_api_key(env_vars: &[(String, String)], is_debug: bool) -> Option<String> {
    if is_debug {
        return env_vars.iter().find_map(|e| {
            if e.0 == "FS_API_KEY" {
                Some(e.1.clone())
            } else {
                None
